...
const grp = new Group();
const functions = fs
    .readdirSync(__dirname.replace("core", "functions"))
    .map((x) => fs.readdirSync(__dirname.replace("core", "functions") + `/${x}`))
    .map((x) => x.join(","))
    .join(",")
    .split(",")
    .map((x) => "$" + x.split(".js")[0]);

fs.readdirSync(__dirname.replace("core", "functions")).map((x) => {
    const filelist = fs.readdirSync(__dirname.replace("core", "functions") + "/" + x);
    maps[x] = filelist.map((y) => y.split(".js")[0]);

    for (const file of filelist) {
        const code = fs.readFileSync(`${__dirname.replace("core", "functions")}/${x}/${file}`).toString();
